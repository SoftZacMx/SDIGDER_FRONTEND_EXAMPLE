...
export class OrderEditionComponent {
  ...
  sale = this.formBuilder.group({
    ...
    payment_method: this.formBuilder.control(0, [Validators.min(1), Validators.required, Validators.maxLength(1)]),
    ...
    this.orderService.updateOrder(order)
      .subscribe({
        ...
        next: (res: IResponse) => {
          ...
          if (res.result) {
            fireSuccessDialog('Orden actualizada con éxito')
        ...
        error: (res: IResponse) => {
          ...
          console.log('createOrder error res', res);
